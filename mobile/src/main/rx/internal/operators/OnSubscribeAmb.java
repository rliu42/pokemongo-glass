...
public final class OnSubscribeAmb<T> implements OnSubscribe<T> {
    ...
    class C11631 implements Action0 {
        C11631() {
        }
    ...
    class C11642 implements Producer {
        C11642() {
        }
    ...
    public void call(Subscriber<? super T> subscriber) {
        subscriber.add(Subscriptions.create(new C11631()));
        for (Observable<? extends T> source : this.sources) {
            if (subscriber.isUnsubscribed()) {
                break;
            }
            AmbSubscriber<T> ambSubscriber = new AmbSubscriber(subscriber, this.selection, null);
        ...
        subscriber.setProducer(new C11642());
    }
