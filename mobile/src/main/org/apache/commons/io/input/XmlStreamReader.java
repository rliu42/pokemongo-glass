...
public class XmlStreamReader extends Reader {
    ...
    public XmlStreamReader(URLConnection conn, String defaultEncoding) throws IOException {
        ...
        BOMInputStream bom = new BOMInputStream(new BufferedInputStream(conn.getInputStream(), BUFFER_SIZE), false, BOMS);
        BOMInputStream pis = new BOMInputStream(bom, true, XML_GUESS_BYTES);
    ...
    private String doLenientDetection(String httpContentType, XmlStreamReaderException ex) throws IOException {
        ...
        if (httpContentType != null && httpContentType.startsWith(Mime.HTML_TEXT)) {
            try {
                calculateHttpEncoding = calculateHttpEncoding("text/xml" + httpContentType.substring(Mime.HTML_TEXT.length()), ex.getBomEncoding(), ex.getXmlGuessEncoding(), ex.getXmlEncoding(), true);
    ...
    private static String getXmlProlog(InputStream is, String guessedEnc) throws IOException {
        ...
        int offset = 0;
        int max = BUFFER_SIZE;
        int c = is.read(bytes, 0, BUFFER_SIZE);
        int firstGT = -1;
