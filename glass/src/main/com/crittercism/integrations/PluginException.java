...
public class PluginException extends Throwable {
    ...
    public static PluginException createUnityException(String message, String stack) {
        ...
        try {
            ...
            dx.m751a("Unable to log unity exception <" + message + "> " + stack, th);
        }
    ...
    private static PluginException unsafeCreateUnityException(String message, String stack) {
        ...
        for (int i = 0; i < split.length; i++) {
            ...
            if (trim2.length() != 0) {
                String str = trim2.split(MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR)[0];
                int lastIndexOf = str.lastIndexOf(".");
                if (lastIndexOf == -1) {
                    dx.m754b("Unable to parse unity exception.  No class and method found for frame frame <" + trim2 + ">" + stack);
                    return null;
                } else if (lastIndexOf == str.length() - 1) {
                    dx.m754b("Unable to parse unity exception.  Method is zero length for frame <" + trim2 + ">" + stack);
                    return null;
                } else {
